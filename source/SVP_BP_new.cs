...
{
    public partial class SVP_BP_new : UserControl
    {
        ...
        {
            return "Blood pressure signal analysis"; //----enter description for your plugin
        }
        ...
        {
            return "Blood Pressure [PhysioCrate]";        //---plugin name, visible in Plugins menu
        }
        ...
        private void btChannels_Click(object sender, EventArgs e)
        {
            ...
            if (sc.ShowDialog() == DialogResult.OK)
            {
                ...
                //signal = new Signal((int)mainView.sampleFrequency, lk.dataCache[0].data, 50, 10);
        ...
        private void SVP_plugin_v3_Load(object sender, EventArgs e)
        {
            ...
            dataGridView2.Columns[1].Name = "pressure [mmHg]";
            dataGridView2.Columns[2].Name = "duration [s]";
            dataGridView2.Columns[3].Name = "start time [s]";
            dataGridView2.Columns[4].Name = "stop time [s]";

            dataGridView3.ColumnCount = 5;
            
            dataGridView3.Columns[0].Name = "mark name";
            dataGridView3.Columns[1].Name = "pressure [mmHg]";
            dataGridView3.Columns[2].Name = "duration [s]";
            dataGridView3.Columns[3].Name = "start time [s]";
            dataGridView3.Columns[4].Name = "stop time [s]";

            dataGridView1.RowCount = 11;
            dataGridView1.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.DisplayedCells;
            dataGridView1.RowHeadersWidth = 150;

            dataGridView1.Rows[0].HeaderCell.Value = "Amplitude mean [mmHg]";
            dataGridView1.Rows[1].HeaderCell.Value = "Amplitude std [mmHg]";
            dataGridView1.Rows[2].HeaderCell.Value = "Cardiac cycle period mean [s]";
            dataGridView1.Rows[3].HeaderCell.Value = "Cardiac cycle period std [s]";
            dataGridView1.Rows[4].HeaderCell.Value = "freq mean [BPM]";
            dataGridView1.Rows[5].HeaderCell.Value = "freq std [BPM]";
            dataGridView1.Rows[6].HeaderCell.Value = "Systole mean [s] ";
            dataGridView1.Rows[7].HeaderCell.Value = "Systole std [s]";
            dataGridView1.Rows[8].HeaderCell.Value = "Diastole mean [s]";
            dataGridView1.Rows[9].HeaderCell.Value = "Diastole std [s]";
        ...
        public void draw_start_marks(List<Anotations> input, List<float> scale, PaintEventArgs e, Pen color)
        {
            ...
            for (int i = 1; i < input.Count-1; i++)
            {
                ...
                //if (input[i].artificial == true && input[i - 1].artificial == false)
                //{
                //    start = i;
                //    continue;
                //}
                ...
                //{
                //   end = i;
                //    e.Graphics.FillRectangle(artArea, start, (int)(0.05f * pbx.Height), end - start, (int)(0.90f * pbx.Height));
                //    continue;
                //}
            }
        ...
        public void CMD_CALL(bool filter)
        {
            lk = linkedChannels[0];
            signal = new Signal((int)mainView.sampleFrequency, lk.dataCache[0].data);
        ...
        //PRESSURE CHANNEL(BP) EXPORT(FiltSig;Marks)
        public string COMMAND_PRESSURE(string parameters)
        {
            try
            {
                mv = (mainView)Application.OpenForms[0];
        ...
        public void cmd_results(List<List<My_selectionMember>> results_inspirium, List<List<My_selectionMember>> results_exspirium)
        {
            ...
            foreach (List<My_selectionMember> list in results_inspirium)
            {
                ...
                globalVariables.setVar("@Pressure_amplitude_avg_interval"+i.ToString(), amplituda.ToString());
                globalVariables.setVar("@Pressure_amplitude_sdev_interval" + i.ToString(), new statistics(null, x, 0, 0, "").dev.ToString());
                ...
                globalVariables.setVar("@Pressure_BPM_avg_interval" + i.ToString(), frekvence.ToString());
                globalVariables.setVar("@Pressure_BPM_sdev_interval" + i.ToString(), new statistics(null, temp_freq, 0, 0, "").dev.ToString());
        ...
        public static string CMDDESCRIPTION_PRESSURE()
        {
            return "Blood pressure analysis. Use : PRESSURE CHANNEL(V1) PARAM(FIR_ORDER;FREQ_CUT) AREA(START_MARK_NAME;WINDOW_LENGTH;ART_MARK) EXPORT(FiltSig;FreqFnc,Marks)";
        }
        ...
        public class Signal
        {
                ...
                for (int i = 0; i < Original.Count; i++)
                {
                    if (AnotationList[i].maximum == true)
                    {
                        ...
                        if (num == null || num.Count == 0)
                            Exspirium.Add(new My_selectionMember(min, max, "Diastole " + Exspirium.Count.ToString(), lk, ampl));
                        else
                        {
                            if(pocet<num.Count)
                            Exspirium.Add(new My_selectionMember(min, max, "Diastole " + num[pocet].ToString(), lk, ampl));
                            pocet++;
                        }
                    }
                }
            ...
            public void getInspirium(signalViewer.graphPanel lk,List<int>num)
            {
                ...
                for (int i = 0; i < Original.Count; i++)
                {
                    if (AnotationList[i].minimum == true)
                    {
                        ...
                        var ampl = Math.Abs(Original[max] - Original[min]);
                        if (num==null || num.Count == 0)
                            Inspirium.Add(new My_selectionMember(min, max, "Systole " + Inspirium.Count.ToString(), lk, ampl));
                        else
                        {
                            Inspirium.Add(new My_selectionMember(min, max, "Systole " + num[pocet].ToString(), lk, ampl));
                            pocet++;
                        }
                    }
                }
           ...
           public filter_FIR filter = new filter_FIR();
            ...
            public void findMax(List<float> input)
            {
                ...
                Parallel.For (Fs, input.Count - Fs, (i) =>
               {

                   if ((input[i - 1] < input[i] && input[i] >= input[i + 1]))
                   { 
                       List<float> temp = new List<float>(input.GetRange((int)(i - Fs / 2), (int)(2 * Fs / 2)));
                       var stat = new statistics(null, temp, 0, 0, "").perc90;
                       if (input[i] > stat)
                       {
            ...
            public void findMin(List<float> input)
            {
                ...
                // Keep one core/CPU free...
                Options.MaxDegreeOfParallelism = Environment.ProcessorCount - 1;
                List<float> X = new List<float>(new float [ input.Count]);
               for(int i=0;i<input.Count;i++)
                { 
                    X[i] = -1 * input[i];
                }
                Parallel.For(Fs, input.Count - Fs, (i) =>
                {
                    ...
                    {
                        List<float> temp = new List<float>(X.GetRange((int)(i - Fs / 2), (int)(2 * Fs / 2)));
                        var stat = new statistics(null, temp, 0, 0, "").perc90;
                        if (X[i] > stat)
                        {
            ...
            public void MinMaxModify(List<selectionMember> insp, List<selectionMember> exsp,List<int> rename_marks_exsp, List<int> rename_marks_insp)
            {
                ...
                foreach (selectionMember e in exsp)
                {
                    ...
                    {
                        index = insp.FindIndex(x => x.info.Contains("Systole " + (cislo+1).ToString()));
                    }
                    else
                    {
                        index = insp.FindIndex(x => x.info.Contains("Systole " + (cislo ).ToString()));
                    }
                    if (index == -1) { }
                    else
        ...
        void resultsPrint(List<List<My_selectionMember>> results,DataGridView a)
        {
            ...
            foreach(List<My_selectionMember> list in results)
            {
                foreach (My_selectionMember mark in list)
                {
                    ...
                    a.Rows[i].Cells[1].Value = signal.Original[mark.rightIndex];
        ...
        public void export_frequency()
        {
            ...
            graphPanel gp = new graphPanel("beat_frequency_function", output, mv);
        ...
        public void export_sys()
        {
            ...
            foreach (List<My_selectionMember> list in results_Exspirium)
            {
                ...
                var z = list.Select(amp => new { amp.amplitude, amp.leftIndex, amp.info }).ToList();
                ...
                for (int g = 1; g < z.Count; g++)
                {
                    ...
                    for (int p = z[g - 1].leftIndex; p < z[g].leftIndex; p++)
                    {
                        output[p] = signal.Original[z[g - 1].leftIndex];
                    }
                }
            }


            graphPanel gp = new graphPanel("Systolic function [mmHg]", output, mv);
        ...
        public void export_dia()
        {
            signal.getInspirium(lk, null);
            signal.getExspirium(lk, null);


            List<List<My_selectionMember>> results_Inspirium = signal._getMy_selectionMemberInRange2(signal.Exspirium, startmark, (int)window.Value, marksList);
            List<float> output = new List<float>(new float[signal.Original.Count]);
        ...
        var z = list.Select(amp => new { amp.rightIndex, amp.leftIndex, amp.info }).ToList();
        ...
        var resultString2 = Int32.Parse(Regex.Match(z[g - 1].info, @"\d+").Value);
                    ...
                    for (int p = z[g - 1].leftIndex; p<z[g].leftIndex; p++)
                    {
                        output[p] = signal.Original[z[g - 1].rightIndex];
                    }
                }
            }


            graphPanel gp = new graphPanel("Diastolic function [mmHg]", output, mv);
